...
const Gallery = () => {
  ...
  const handleGridPrev = () => {
    setGridCurrentIndex((prev) => 
      prev - imagesPerPage >= 0 ? prev - imagesPerPage : Math.max(0, (totalPages - 1) * imagesPerPage)
    );
  };
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {getCurrentGridImages().map((image, index) => (
                <div
                  ...
                  className="aspect-square relative overflow-hidden rounded-lg shadow-md hover:shadow-lg transition-all duration-300 group cursor-pointer"
                >
                  <Image
                    src={image.src}
                    alt={image.alt}
