  ...
  return (
    <div>
      ...
      <div className="relative h-screen lg:min-h-full lg:max-h-[1000px] w-full">
        <div className="absolute inset-0 bg-[url('/IMG_0107.JPG')] bg-cover bg-center">
            ...
            {departments.map((dept) => (
              <button
                ...
                className={`px-6 py-3 rounded-full text-sm md:text-base transition-all duration-300 
                  ${activeTab === dept.id ? "bg-blue-700 text-white" : "bg-blue-50 text-gray-700 hover:bg-blue-100"}`}
              >
                {dept.name}
              </button>
            ))}
            ...
            <div className="max-w-6xl mx-auto">
              <div className="grid md:grid-cols-2 gap-8 items-center">
                ...
                <div className="space-y-4">
                  <h3 className="text-2xl font-bold text-gray-900">{activeDepartment.title}</h3>
                  <p className="text-gray-600 leading-relaxed">{activeDepartment.description}</p>
                </div>
              </div>
            </div>
          )}
