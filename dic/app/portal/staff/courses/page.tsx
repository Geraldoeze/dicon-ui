...
const Courses = () => {
  const { data: staffCourses, error, isLoading } = useQuery({
    ...
    <div className='bg-slate-50'>
      ...
      <div className="max-w-70vw mx-auto rounded bg-white">
          <div className="p-10">
            ...
            <p className='text-base text-gray-700'>View the courses you are taking</p>
            </div>
            <div className="border-y-2 my-5 gap-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
                {staffCourses?.data.map((course)=>(
                  <div className="border-e-2 border-b-2 border-gray-100 min-w-[350px] p-5" key={course.course_id}>
                      <div className="flex flex-col space-y-3 my-2">
                        <div className="flex flex-col gap-y-2">
                          <h1 className='text-xl font-semibold'>{course.course_code}</h1>
                      ...
                      <a href={`/portal/staff/courses/${course.course_id}`} className='space-x-2 flex font-medium'> <span>Details</span> <ArrowRight/></a>
                      </div> 
                  </div>
                ))
